...
| QA-155 | High | Low | Audit: Quarterly data integrity audit — scan all records in `propSearch.db` and `master.jsonl` for hallucinated IDs (`a1b2c3d4-` pattern), missing mandatory fields, and records with `is_estimated=false` but no source citation. Report findings to user. | Todo | UI/UX QA | None | 2026-03-30 |
