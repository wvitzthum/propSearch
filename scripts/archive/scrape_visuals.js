...
async function extractFlaresolverrData(html, source) {
  ...
  if (source === 'Zoopla') {
    ...
    // Unescape all \" → " before pattern matching — do this once for the full document
    const unescHtml = html.split('\\\"').join('"');

    // DAT-205: Price history, date first listed, and listing ID — extracted from full HTML
    // (not scoped to floorArea ctx so properties without sqft still get price history)
    const priceHistMatch = unescHtml.match(/"priceHistory"\s*:\s*(\[[^\]]*\])/);
    let priceHistory = [];
    if (priceHistMatch) {
      try { priceHistory = JSON.parse(priceHistMatch[1]); } catch(e) { priceHistory = []; }
    }
    const dateFirstListed = unescHtml.match(/"dateFirstListed"\s*:\s*"([^"]+)"/)?.[1] || null;
    const zooplaListingId = unescHtml.match(/"listingId"\s*:\s*"?([^",}]+)"?/)?.[1] || null;
    Object.assign(result, { priceHistory, dateFirstListed, listingId: zooplaListingId });

    // Find the listingData section with counts, floorArea, tenure etc.
    const idx = unescHtml.indexOf('"floorArea"');
    if (idx >= 0) {
      const ctx = unescHtml.substring(Math.max(0, idx - 300), idx + 2000);
    ...
    if (result.source === 'Rightmove') {
      ...
      try {
        ...
        result = {
          ...
          // DAT-205: Zoopla price history fields
          priceHistory: fsData.priceHistory ?? result.priceHistory,
          dateFirstListed: fsData.dateFirstListed ?? result.dateFirstListed,
          listingId: fsData.listingId ?? result.listingId
        };
        console.log('[Zoopla] FlareSolverr extraction complete:', {
          ...
          dateFirstListed: result.dateFirstListed,
          priceHistoryCount: result.priceHistory?.length ?? 0
        });
      } catch (fsErr) {
