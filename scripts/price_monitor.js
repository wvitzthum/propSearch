...
function insertPriceSnapshot(propertyId, price, date) {
...
function runPriceMonitor() {
  ...
  for (const prop of properties) {
    ...
    if (!latestHistory || latestHistory.price !== prop.list_price) {
      const inserted = insertPriceSnapshot(prop.id, prop.list_price, TODAY);
      if (inserted) {
        snapshotsAdded++;
        ...
        if (latestHistory && prop.list_price < latestHistory.price) {
          ...
          // Calculate days since reduction (from today back to the latest previous price date)
