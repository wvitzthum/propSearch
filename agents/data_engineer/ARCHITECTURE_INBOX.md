...
- **Path:** `/workspaces/propSearch/data/inbox/`
...
A script (or agent logic) reads the latest Inbox files and transforms them into the propSearch schema (`property.schema.json`).
...
- [ ] Update `data/.gitignore` to ensure raw files are tracked but managed (if large, consider `.gitattributes` or external storage, but for propSearch we keep it in-repo for now).
